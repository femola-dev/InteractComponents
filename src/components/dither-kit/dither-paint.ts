...
export type PaintOpts = {
  ...
  /** Recolours the dither body only, leaving the top outline on `seed`. Lets a
   *  stretch of the fill read as de-emphasised without breaking the series line
   *  that traces it. Defaults to `seed`. */
  bodySeed?: Seed
}
...
export function paintColumn(
  ...
  floor: number,
  seed: Seed,
  { variant, intensity, dim, stacked, sparse = 0, bodySeed = seed }: PaintOpts
) {
  ...
  for (let y = t; y < f; y++) {
    ...
    octx.fillStyle = rgb(bodySeed.fill, 1, alpha)
